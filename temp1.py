...
with Tello(my_tellos) as fly: # Use Tello as a Context Manager to ensure safe landing in case of any errors 
    ...
    # fly.forward(50) # Single command for all Tellos to fly forward by 50cm with 
    fly.forward(50) # Single command for all Tellos to fly forward by 50cm with 
    fly.sync_these(): # Keep the following commands in-sync, even with different commands for each Tello 
