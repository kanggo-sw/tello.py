    ...
    def _receive_thread(self):
        while True:
            try:
                ...
                ip = "".join(str(ip[0]))
                if (
                    hasattr(self.response, "upper")
                    and self.response.upper() == "OK"
                    and ip not in self.tello_ip_list
                ):
...
class Tello(object):
    ...
    def __init__(self, tello_ip, instance: _Tello):
        self.tello_ip = tello_ip
        self.instance = instance

    def send_command(self, command):
        return self.instance.send_command(command, self.tello_ip)
