...
class Tello(object):
    """ Abstract class providing a simpler, user-friendly interface to Kernel and Tello classes.

        Tello is dependent on Kernel, which itself uses Tello and TelloCommand.

        Tello is intended to be used as a Context Manager, i.e. to be initialized using a "with" statement, e.g.:
            with Tello([sn1, sn2]) as fly:
                fly.takeoff()
    """
    ...
    def __enter__(self):
        """ (ContextManager) Called when Tello is initiated using a with statement. """
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.tello_kernel.close_connections()
    ...
    def sync_these(self) -> None:
        """ Synchronise the commands within the "with" block, when this is used as a Context Manager.
            ...
            sync_these() is intended to be used as a Context Manager, i.e. to initialize using a "with" statement, e.g.:
                with fly.sync_these():
                    fly.left(50, 1)
                    fly.right(50, 2)
    ...
    def flight_complete(self, tello: int) -> None:
        ...
        """
        self.tello_kernel.get_tello(tello).flight_complete = True
